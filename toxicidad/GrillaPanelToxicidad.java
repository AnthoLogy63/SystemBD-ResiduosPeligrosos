...
public class GrillaPanelToxicidad extends JPanel {
    private JTable table;

    public GrillaPanelToxicidad() {
        setBackground(Color.decode("#FFFFFF"));
        setBorder(BorderFactory.createTitledBorder("Tabla_Toxicidad"));
        ...
        DefaultTableCellRenderer renderer = new DefaultTableCellRenderer() {
            Color even = Color.decode("#FFFFFF");
            Color odd = Color.decode("#f7f7f7");
            Color sel = new Color(200, 220, 255);

            @Override
            public Component getTableCellRendererComponent(JTable t, Object val,
                    boolean selRow, boolean hasFocus, int row, int col) {
                super.getTableCellRendererComponent(t, val, selRow, hasFocus, row, col);
                setBackground(selRow ? sel : (row % 2 == 0 ? even : odd));
        ...
        colModel.getColumn(1).setPreferredWidth(300);  // Nombre
        colModel.getColumn(2).setPreferredWidth(120);  // Estado
