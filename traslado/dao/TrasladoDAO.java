...
public class TrasladoDAO {
    ...
    public List<TrasladoModel> findAll() throws SQLException {
        ...
        String sql = "SELECT * FROM R1T_TRASLADO ORDER BY CAST(TraFecEnv AS DATE) ASC";
    ...
    public TrasladoModel findById(String empNif, int resCod, String traFecEnv, String destCod) throws SQLException {
        String sql = "SELECT * FROM R1T_TRASLADO WHERE EmpNif = ? AND ResCod = ? AND TraFecEnv = ? AND DestCod = ?";
    ...
    public void insert(TrasladoModel t) throws SQLException {
        String sql = "INSERT INTO R1T_TRASLADO (EmpNif, ResCod, TraFecEnv, DestCod, TraEnv, TraFecLlega, TraTrat, TraCant, TraObs, TransCod, EstCod) " +
                     "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public void update(TrasladoModel t) throws SQLException {
        String sql = "UPDATE R1T_TRASLADO SET TraEnv = ?, TraFecLlega = ?, TraTrat = ?, TraCant = ?, TraObs = ?, TransCod = ?, EstCod = ? " +
    ...
    private boolean updateEstado(String empNif, int resCod, String traFecEnv, String destCod, char nuevoEstado) throws SQLException {
        String sql = "UPDATE R1T_TRASLADO SET EstCod = ? WHERE EmpNif = ? AND ResCod = ? AND TraFecEnv = ? AND DestCod = ?";
        try (Connection conn = DBConnectionMySQL.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public List<String> getEmpresasActivas() throws SQLException {
        ...
        String sql = "SELECT EmpNif FROM R1M_EMPRESA WHERE EmpEst = 'A' ORDER BY EmpNif";
        try (Connection conn = DBConnectionMySQL.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) lista.add(rs.getString("EmpNif"));
        }
        return lista;
    }
    ...
    public List<Integer> getResiduosActivos() throws SQLException {
        ...
        String sql = "SELECT ResCod FROM R1T_RESIDUO WHERE ResEst = 'A' ORDER BY ResCod";
        try (Connection conn = DBConnectionMySQL.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) lista.add(rs.getInt("ResCod"));
        }
        return lista;
    }
    ...
    public List<String> getDestinosActivos() throws SQLException {
        List<String> lista = new ArrayList<>();
        String sql = "SELECT DestCod FROM R1M_DESTINO WHERE DestEst = 'A' ORDER BY DestCod";
        try (Connection conn = DBConnectionMySQL.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) lista.add(rs.getString("DestCod"));
        }
        return lista;
    }
    ...
    public List<String> getEnvasesActivos() throws SQLException {
        List<String> lista = new ArrayList<>();
        String sql = "SELECT EnvCod FROM GZZ_ENVASE WHERE EnvEst = 'A' ORDER BY EnvCod";
        try (Connection conn = DBConnectionMySQL.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) lista.add(rs.getString("EnvCod"));
        }
        return lista;
    }
    ...
    public List<String> getTratamientosActivos() throws SQLException {
        List<String> lista = new ArrayList<>();
        String sql = "SELECT TratCod FROM GZZ_TRATAMIENTO WHERE TratEst = 'A' ORDER BY TratCod";
        try (Connection conn = DBConnectionMySQL.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) lista.add(rs.getString("TratCod"));
        }
        return lista;
    }
    ...
    public List<String> getTransportistasActivos() throws SQLException {
        List<String> lista = new ArrayList<>();
        String sql = "SELECT EmpTransNif FROM R1M_TRANSPORTISTA WHERE EmpTransEst = 'A' ORDER BY EmpTransNif";
        try (Connection conn = DBConnectionMySQL.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) lista.add(rs.getString("EmpTransNif"));
        }
        return lista;
    }

    public double obtenerCantidadAcumuladaPorDestino(String destCod) throws SQLException {
        String sql = "SELECT SUM(TraCant) FROM R1T_TRASLADO WHERE DestCod = ? AND EstCod = 'A'";
        try (Connection conn = DBConnectionMySQL.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
